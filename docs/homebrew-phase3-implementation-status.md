...
- 变量生成 prompt 仍需继续收口：关闭生存系统时不应注入生理难度/饱腹口渴/休息代谢；现代默认变量示例应裁掉跨题材物品表和旧锚点。
- 开局规划初始化请求目前复用了主剧情身份外壳，容易被误判为“地图前又跑了一次主剧情”。后续应切为专用规划身份，并在诊断中明确标注 `opening_planning`。
- 开局链路性能进入后续优化队列：全阶段都走 `deepseek-v4-pro` 时完整开局实测约 468 秒，主要问题是多重模型阶段串行、变量/规划 prompt 重、工具阶段输出预算过宽。详细方案记录在 `docs/homebrew-phase3.4-fix-plan.md`。
