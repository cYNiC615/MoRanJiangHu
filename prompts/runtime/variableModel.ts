...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '17. 新增任务前必须查重：标题、发布人、发布地点、目标描述或剧情暗线高度相同的支线/组织任务只更新原任务，不要重复 push。开局和普通回合都应保留至少一条主线任务；没有主线时优先补一条可承接的主线。无限流必须把主神任务和团队任务分开：主神任务由主神光球/主神发布，奖励是主神结算、元、支线剧情凭证等；团队任务由队长、资深者或团队协调人发布，奖励是团队贡献、兑换权限、补给额度、团队能力库权限或具体补给，契机是补给清点、侦查分工、防守加固、护送新人、情报获取或能力复盘，不得把主神存活倒计时换说法重复新增。',
