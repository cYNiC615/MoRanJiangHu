...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '10. 正文可用代称，变量层必须使用 2-4 个中文汉字真实姓名；代称、身份称呼、外貌描述应优先写入 `身份/简介/记忆`，只有确有旧称、化名、曾用称呼时才写 `曾用名`，不要给每个 NPC 强行生成曾用名。新建女性 NPC 时遵循本回合注入的【女性 NPC 命名风格】：默认现代架空都市感，年龄、职业和成熟感靠称谓、身份、行为与语气体现，不靠老派姓名体现；有独立对白框的女性、长期关系对象、关键承接对象，要按 NPC 协议补齐缺失的外貌、身材、衣着、称呼、关系突破条件、私密档案和名器档案。已有值默认保留沿用，不得随意改写；仅在本回合正文明确坐实质变事件后才改写对应字段。',
            ...
            '17. 新增任务前必须先做正式目标准入和查重：只有主线/当前章目标、重要关系或红颜阶段目标、明确委托或报酬事项、有风险/时限/代价/失败后果的目标、玩家明确要求持续追踪的目标、导演模块标记的重要目标，才允许写入 `任务列表`。标题、发布人、发布地点和目标描述高度相同的任务只更新原任务，不要重复 push。普通跑腿、顺路调查、低价值找物、闲聊传闻、气氛性线索和玩家追问细节时临场生成的枝节，不写入 `任务列表`；需要承接时留给正文、短期记忆、`剧情规划` 或女主规划。主线不是短待办：若写入主线任务，每条必须包含阶段目标、明确阻力、变化空间和具体锚点（人物/地点/事件/物品至少一项），不得只写“确认目标 / 继续生活 / 等待发展 / 了解情况”这类空泛目标。',
            '17.0 现代都市首条主线若确实写入 `任务列表`，必须是长期压力下的短周期阶段任务：目标列表必须在游戏内 1 周以内可完成或至少阶段性推进，锚定当前身份处境、现实压力、具体关系或资源入口；“完成整个学期 / 顺利毕业 / 长期立足城市 / 建立长期人脉根基”不能直接写成首条目标，只能作为背景压力或后续规划。',
            '17.0.1 【禁止任务套话】任务的目标描述、触发条件和奖励说明必须基于当前场景、人物关系、现实压力和已发生事件生成，严禁使用与当前局面无关的通用套话。例如：不得生成"探索区域并收集情报"、"消灭威胁并确保安全"、"与队友汇合并确认下一步行动"、"寻找物资并建立据点"、"调查异常现象并记录数据"等每次开局都一模一样的模板化描述。每个任务必须包含至少一个具体的地名、NPC名、物品名或剧情事件作为锚点。',
            ...
            '17.1 【禁止势力/组织通用名称】地图节点的 `控制势力`、`势力标签` 必须使用具体的、有辨识度的中文名称。严禁使用"团队A"、"团队B"、"小队1"、"队伍A"、"组织1"等编号式通用名称；应由世界观、地点职责、组织功能和剧情事实生成有特色的名称。',
            ...
            '21. `社交[i].姓名` 必须是该人物的真实姓名，使用 2-4 个中文汉字；新女性角色遵循现代架空都市感命名，同一存档内不得重复。已经存在的姓名、玩家手动改过的姓名、正文已稳定承接的姓名必须原样保留，不得为了风格统一或“更合理”而改名；即使老存档里已有不理想姓名，也不要主动重命名。只有当前姓名是占位/代称，且本回合正文明确揭示真名时，才允许 `set 社交[i].姓名`。禁止把“自己/他/她/对方/那人/黑衣女子/某护卫/自己已经没有”等代称、身份、短句或正文动作写进姓名。正文里可以继续用代称，但变量里要建立“真实姓名 + 身份/简介/记忆”的稳定对应；若只知道代称，也要先为该 NPC 起一个可持续使用的真实姓名，把代称放到身份、简介或记忆。`曾用名` 只用于真实存在的旧称、化名、曾用称呼，不得为了凑字段给每个 NPC 都写。',
