...
export const 解析正文日志文本 = (
    ...
    return 规范化对白日志(
        logs.filter(item => typeof item.text === 'string' && item.text.trim().length > 0),
        { declaredNames: options?.declaredNames }
    );
};
...
export const 净化角色对白行 = (
    ...
    return 规范化对白日志(result, { declaredNames: options?.declaredNames });
};
...
export const 执行正文润色 = async (
    ...
    const declaredNames = new Set<string>([
        playerDisplayName,
        ...(Array.isArray(deps.社交) ? deps.社交 : [])
            .map((npc: any) => (typeof npc?.姓名 === 'string' ? npc.姓名.trim() : ''))
            .filter(Boolean)
    ]
        .map((name) => 规范化正文发送者(name))
        .filter(Boolean));
    ...
    const polishOutputContract = [
        ...
        '8) 物品、角色天赋、出身背景等可查看档案引用统一使用《名称》，例如《随身短刃》《证据录音》《安全屋直觉》；禁止写成【名称】，因为【...】只用于旁白、角色名和判定标签。',
