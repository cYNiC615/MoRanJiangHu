...
export const 构建女性姓名黑名单提示词 = (): string => [
    ...
    '- 年龄、职业和成熟感通过称谓、身份、行为、语气体现，不靠老派姓名体现。',
    '- 风格参考：林知夏、许清禾、周以宁、沈嘉宁、宋雨禾、方静宜、顾明澜、林晚晴、陆星遥、江映雪、高艺敏、刘佳玮、孙雪琳、徐静宜、李绒、马可欣、张伊丹。',
    '- 示例仅用于把握语感，不要直接复读；每次生成时应结合角色家庭、职业、地区和人物气质另起新名。',
    '- 正文对白框 sender、`<变量规划>` 中的人物称呼、变量命令里的 `社交[i].姓名` 必须完全一致。',
    '- 已存在于社交档案、玩家手动修改或正文已点名的人物姓名必须原样保留；不要为了统一风格而改旧角色。',
    '- 临时代称、身份称呼或外貌描述不得写进姓名，可写入身份、简介或记忆；只有确有旧称、化名、曾用称呼时才写 `曾用名`，不要给每个 NPC 强行生成曾用名。'
].join('\n');

/*
 * 保留旧函数与黑名单表用于诊断、旧测试和未来可能的迁移工具。
 * 当前运行时不再因为模板化女性姓名触发整轮重试，命名审美主要交给提示词约束。
 */
const 提取命中新女性角色姓名黑名单_旧硬拦截 = (params: {
    response?: any;
    commands?: any[];
    currentSocial?: any[];
    includeLogSenders?: boolean;
