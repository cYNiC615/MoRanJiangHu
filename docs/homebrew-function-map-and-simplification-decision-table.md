...
| 武侠/修仙专属入口 | Phase 1 已完成：功法、技艺、门派、境界配置、新建角武侠入口、主系统/酒馆上下文、修炼体系默认开关和 `玩家门派` 命令根维护已断 | 不可达前端进 Phase 1.5；模型/prompt/存档字段后续深删 |

Phase 1 收口前只需要继续做三类检查：

1. 从当前桌面主流程确认废弃功能是否仍有真实可触发入口或顶层副作用。
2. 从 active prompt/schema/命令过滤确认 AI 是否仍会主动维护废弃结构化状态。
3. 完成一轮浏览器 smoke / 人工游玩检查；当前静态测试、搜索审计和 build 已通过，本地 HTTP 预览可达，浏览器自动化 smoke 仍待补。
