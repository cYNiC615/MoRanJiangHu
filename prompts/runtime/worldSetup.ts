...
export const 构建世界观难度摘要 = (promptPool: 提示词结构[]): string => {
    ...
    const lines = [
        '【当前世界观生成难度摘要】',
        '- 本摘要只用于世界母本阶段，约束轻量日常张力、可玩风险、资源压力、失败代价与日常压力；不要求展开经济、制度或社会运行报告，不携带完整判定、数值或细则协议。'
    ];
    if (gamePrompt) {
...
const 构建世界观题材提示词 = (openingConfig?: OpeningConfig | null): string => {
    ...
    return [
        ...
        '- 默认体验偏后宫恋爱轻喜剧与都市日常；除非玩家明确要求，不把犯罪、黑市、家暴、勒索、政治丑闻或商业阴谋写成主舞台。',
        '- 普通现代常识默认成立，不作为篇幅重点；篇幅优先给本局独特规则、独特规则造成的社会后果、关系习惯、地点差异和可长期复用的世界事实。',
        '- 玩家自定义的特殊规则要落到关系节奏、家庭选择、同居/约会、育儿/亲密关系、日常场景和常见选择的二阶影响，不只复述规则。',
        ...
        '- 经济和货币规则只作为结算与行动边界，不展开宏观经济循环、城市财政、行业报告或社会学分析。',
...
const 构建世界锚点硬边界 = (openingConfig?: OpeningConfig | null): string => {
    ...
    if (是现代都市题材(openingConfig)) {
        return [
            ...
            '- 世界应优先提供可进入地点、可遇见人物、常见日常场景、关系/女主土壤和可被玩家追问的世界内秘密。',
            '- 默认把高价值机会、关键资源、关系入口和信息差转化为恋爱、社交、家庭、工作/学业或轻量麻烦中的可玩张力。',
            '- 世界保持回报与代价匹配，不写成低风险高回报的福利场，也不默认写成高压社会冲突舞台。'
        ].join('\n');
    }
    return [
...
const 构建世界观成长一致性提示 = (openingConfig?: OpeningConfig | null): string => {
    ...
    if (是现代都市题材(openingConfig)) {
        return [
            '2. 成长体系一致性',
            '   - 本存档世界必须兼容现代都市成长体系。',
            '   - 高价值机会、关键资源、关系入口、信息差和现实资源应稀缺，高收益应对应代价、误会、关系压力、时间成本或明确社会后果。',
            '   - 设定与现有数值哲学保持一致，例如低风险地区不泛滥高价值捷径，关键资源和圈层影响力受控，冒进收益伴随代价；但经济、制度、法律和交通只作为行动边界，不压过恋爱轻喜剧默认体验。'
        ].join('\n');
    }
    return [
        '2. 成长体系一致性',
