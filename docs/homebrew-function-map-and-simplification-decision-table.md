...
Phase 1 当前实现侧已完成；Phase 1.5 尚未开始。进入 Phase 1.5 前建议先做一轮人工游玩确认，重点看主聊天、存档、世界书、提示词、记忆和设置是否仍符合实际使用习惯。
...
Phase 1 当前状态：实现侧已完成，Phase 1.5 未开始。
...
Phase 1.5 在 Phase 1 实现侧完成后开始，但当前尚未启动。它只清理已经不可达的前端代码，不把深层后端、模型、prompt、storage 强迁移混进同一刀。进入前建议先完成一轮人工游玩确认。
...
当前最合理的下一步是人工游玩确认 Phase 1：用真实本地设置和存档走一轮主聊天、存档、世界书、提示词、记忆和设置检查。确认没有问题后，再进入 Phase 1.5，选择一个不可达前端模块做第一刀。
