...
- 新建游戏的世界观选择界面仍显示“市场入口”等前端文本块；后续现代默认化或新建向导收口时删除这类市场展示块。
- 游戏内右侧菜单的“随身装备”和“随身物品”货币显示不合格：目前只显示描述性文本，且把 `baseAmount` 这类内部字段名暴露给玩家；后续应显示具体货币数字和玩家可读单位。
- 开局变量生成输出疑似被截断；先按模型/输出预算问题记录，后续需要结合原始响应、max tokens 和阶段重试逻辑再判断。
- 主游戏界面展开“队列”后，点击开局变量生成的“重新生成”没有反应；随后收起队列时主界面正文会消失。后续需优先审计队列按钮事件、开局阶段重试入口和聊天正文展示状态。
- 红颜卷宗/女主规划没有任何内容写入时仍显示红点；后续应把红点条件收紧为确有可读内容或确有待处理更新。
