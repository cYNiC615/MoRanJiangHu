...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '12. 货币语义必须跟随现代都市/近未来题材并保持底层统一换算：日常优先使用人民币、电子支付、合同、工资、债务、信用额度；近未来或异常交易可使用组织信用、研究额度、异常物资、情报、营地信用等。若当前开局运行配置存在 `modeRuntimeProfile.economy.currencySystem`，所有余额、任务奖励和货币文本解析都要优先识别其中 `units[].name/symbol/aliases`，并按 `baseRate` 折算进 `角色.金钱.baseAmount`；普通回合不要擅自改写或重建 `currencySystem`。若没有显式 `currencySystem`，继续使用旧三层货币 fallback。`currencySystem` 是可选结构：`{ id, name, baseUnitId, formatStyle?: "single"|"compound", units:[{ id, name, symbol?, baseRate, order, aliases? }] }`；所有 `baseRate` 必须是正整数，baseUnit 的 `baseRate` 必须为 1，`order` 越大表示越高等级货币，普通世界 1-4 个单位即可，交易计算由程序完成，不要在剧情中手动乱算汇率。若金钱以现金、储值卡、票据、营地配给券等实际载体直接交到主角手里，优先落为 `角色.物品列表` 中的 `货币:*` 物品；若只是电子转账、信用点记账、合同工资、系统结算或其他无实体载体余额，则优先写入 `角色.金钱`，并保留 `{ 金元宝, 银子, 铜钱, baseAmount? }` 兼容字段。',
            ...
            '17.1.1 【禁止势力/组织通用名称】地图节点的 `控制势力`、`势力标签` 必须使用具体的、有辨识度的中文名称。严禁使用"轮回小队A"、"轮回小队B"、"小队1"、"队伍A"等编号式通用名称。无限流模式的势力应参考经典无限流原著（如《无限恐怖》《死亡万花筒》《全球高武》等）的命名风格，使用具体组织名如"天神队"、"恶魔队"、"中洲队"、"北冰洲队"等，或由世界观和剧情自动生成有特色的名称。无限流世界观中主神/主神空间只有一个，不得在多个不同地点节点中重复出现"主神"作为控制势力。',
...
export const 构建变量模型系统提示词 = (options?: {
    ...
    构建变量模型职责提示词({
        ...
        cultivationSystemEnabled: options?.cultivationSystemEnabled === true
    })
].join('\n'));
