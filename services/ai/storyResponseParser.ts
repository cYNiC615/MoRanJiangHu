...
const 初始化泄露行规则 = /^(?:[-*]\s*)?(?:\*\*)?(基础信息|基础属性|境界系统|属性数值|六维|天赋列表|身体状态|资产|资产与物品|装备与物品|装备中|物品列表|坐标|时间|地点层级|天气|玩家组织|任务列表|NPC\d+|Item\d+|Task\d+)(?:\*\*)?\s*[：:]/;
const 初始化泄露列表行规则 = /^(?:[-*]\s*)?(?:\*\*[^*]+?\*\*|(?:姓名|年龄|性别|身份|称号|境界|境界层级|内力|精力|饱腹|口渴|头部|胸部|腹部|左手|右手|左腿|右腿|金钱|装备中|物品列表|地点层级|天气|玩家组织)\b|[\[\{]?(?:NPC|Item|Task)\d+|\[\d+\]\s*名称[：:])/;
